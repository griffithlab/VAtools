...
class VcfExpressionAnnotatorTests(unittest.TestCase):
    ...
    def test_stringtie_gtf_file_with_gene_mode_errors(self):
        temp_path = tempfile.TemporaryDirectory()
        os.symlink(os.path.join(self.test_data_dir, 'input.vcf'), os.path.join(temp_path.name, 'input.vcf'))
        command = [
            os.path.join(temp_path.name, 'input.vcf'),
        ...
        with self.assertRaises(Exception) as context:
            vcf_expression_annotator.main(command)
        temp_path.cleanup()
        self.assertTrue('ERROR: You provided a GTF file, but stringtie gene annotation is typically in tsv format. Did you mean to provide the "transcript" option?' == str(context.exception))

    def test_stringtie_tsv_file_with_transcript_mode_errors(self):
        temp_path = tempfile.TemporaryDirectory()
        os.symlink(os.path.join(self.test_data_dir, 'input.vcf'), os.path.join(temp_path.name, 'input.vcf'))
        command = [
            os.path.join(temp_path.name, 'input.vcf'),
        ...
        with self.assertRaises(Exception) as context:
            vcf_expression_annotator.main(command)
        temp_path.cleanup()
        self.assertTrue('ERROR: You provided a TSV file, but stringtie transcript annotation is typically in gtf format. Did you mean to provide the "gene" option?' == str(context.exception))
